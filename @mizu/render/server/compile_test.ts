...
if (typeof (globalThis as { Deno?: { bundle?: unknown } }).Deno?.bundle === "function") {
  ...
  test("`Server.compile()` bundles additional directives passed through the `modules` option", async () => {
    const mizu = new Server({ directives: ["@mizu/mizu/compile", "@mizu/test"] })
    const [, script] = split(await mizu.compile(`<main *mizu.compile><p ~test.text="'foo'"></p></main>`, { select: "main", modules: { [import.meta.resolve("@mizu/test")]: _test } }))
  ...
  test("`Server.compile()` with `render` mode removes the comments generated by directives", async () => {
    const mizu = new Server({ context: { shown: false, items: ["a", "b"] } })
    const [before] = split(await mizu.compile(`<main *mizu.compile="render"><p *if="shown"></p><!--kept--><li *for="const item of items" *text="item"></li></main>`, { select: "main" }))
    expect(before).toBe(`<main><!--kept--><li *text="item">a</li><li *text="item">b</li>`)
  }, { permissions: "inherit" })
