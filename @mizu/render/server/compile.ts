...
export function entrypoint(renderer: Renderer, element: HTMLElement, { context, modules, script, warn }: CompileOptions): string {
  ...
  return [
    ...
    `    return renderer.render($element, { reactive: true, ...options, context: new Context({ ...$context, ...context }), state: { $renderer: "client", [renderer.internal("compiled")]: true, ...options.state } })`,
    "  },",
    "}",
    script.content,
  ].join("\n")
...
/** Remove the comments generated by directives from an element. */
export function clean(renderer: Renderer, element: HTMLElement): void {
  for (const node of Array.from(element.childNodes) as Array<HTMLElement | Comment>) {
    if ((renderer.isComment(node)) && ((renderer.cache("*")?.has(node)) || (/^\[\/\S+\]$/.test(node.data)))) {
      node.remove()
      continue
    }
