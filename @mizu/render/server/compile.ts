...
import type { Cache, Context, Directive, Renderer } from "@mizu/internal/engine"
import { hydratable } from "@mizu/mizu/compile/hydrate"
import { _for as iterative } from "@mizu/for"
import { _if as conditional, type Cache as Templates } from "@mizu/if"
...
export type CompileOptions = {
  ...
  /** Whether the element must be hydrated before being rendered. */
  hydrate: boolean
  /** Warning callback. */
...
export function entrypoint(renderer: Renderer, element: HTMLElement, { context, modules, script, hydrate, warn }: CompileOptions): string {
  ...
  return [
    ...
    hydrate ? `import { hydrate as $hydrate } from ${quote(import.meta.resolve("@mizu/mizu/compile/hydrate"))}` : "",
    ...
    hydrate ? "    $hydrate(renderer, $element)" : "",
...
/**
 * Preserve the elements commented out by directives so they can be restored by a client-side hydration.
 *
 * The commented out element is moved into an inert `<template>`, and the nodes generated after it are delimited by a closing marker.
 */
export function dehydrate(renderer: Renderer, element: HTMLElement): void {
  for (const node of Array.from(element.childNodes) as Array<HTMLElement | Comment>) {
    if (renderer.isHtmlElement(node)) {
      dehydrate(renderer, node)
      continue
    }
    const original = renderer.isComment(node) ? renderer.cache("*")?.get(node) as Nullable<HTMLElement> : null
    if (!original) {
      continue
    }
    const generated = (last(renderer, node) ?? node) as unknown as ChildNode
    generated.parentNode?.insertBefore(renderer.document.createComment(`[/${hydratable}]`), generated.nextSibling)
    const template = renderer.document.createElement("template") as HTMLTemplateElement
    renderer.setAttribute(template, hydratable, node.data)
    template.content.appendChild(original)
    ;(node as unknown as ChildNode).replaceWith(template)
  }
}

/** Resolve the last node generated after a comment. */
function last(renderer: Renderer, comment: Comment) {
  const iterated = [...(renderer.cache<Cache<typeof iterative>>(iterative.name)?.get(comment)?.items.values() ?? [])].at(-1)
  if (iterated) {
    return (iterated.end ?? iterated.nodes.at(-1)) as Nullable<Node>
  }
  const templated = renderer.cache<Templates>(conditional.name)?.templates.get(comment)
  return (templated?.end.parentNode ? templated.end : null) as Nullable<Node>
}
