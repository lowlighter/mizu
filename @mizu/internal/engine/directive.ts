...
import type { AttrTypings, Context, Renderer, State } from "./renderer.ts"
...
> =
  & {
     ...
     *   }
     * } as const satisfies Directive<{ Typings: typeof typings}>
     * ```
     */
     ...
     *   },
     * } as const satisfies Directive<{ Cache: WeakSet<HTMLElement | Comment> }>
     * ```
     */
  ...
  & (Definition["Prefix"] extends true ? {
      /**
       * Directive prefix.
       *
    ...
    : {})
  & (Definition["Name"] extends RegExp ? {
      /**
       * Directive prefix.
       *
       * It is used as a hint for {@linkcode Renderer.parseAttribute()} to strip prefix from {@linkcode https://developer.mozilla.org/docs/Web/API/Attr/name | Attr.name} when parsing the directive.
    ...
    // deno-lint-ignore ban-types
    : {})
  & (Definition["Default"] extends true ? {
      /**
       * Default value.
       *
       ...
       *   }
       * } as const satisfies Directive<{ Default: true }>
       * ```
       */
