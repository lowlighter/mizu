...
test("`evaluates()` supports `permissions` options", async () => {
  ...
  await expect(evaluate("Deno.osRelease()", null, { permissions: { sys: true } })).resolves.toBe(Deno.osRelease())
})
...
test("`evaluates()` supports `permissions` options and evaluates expressions in function context", async () => {
  ...
  await expect(evaluate("return Deno.osRelease()", null, { context: "function", permissions: { sys: true } })).resolves.toBe(Deno.osRelease())
})
...
test("`evaluates()` supports `permissions` and `imports` options", async () => {
  ...
  await expect(evaluate("foo.default", null, { permissions: { sys: true }, imports: { foo: "data:application/javascript,export default Deno.osRelease()" } })).resolves.toBe(Deno.osRelease())
})
