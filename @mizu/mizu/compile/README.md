...
> [!NOTE]
> In the `render` mode, the elements commented out by directives _(e.g. by [`*if`](#if))_ are preserved in inert `<template>` elements, and restored before the element is rendered again client-side.
